...
def calculate_corner_speeds(json_path):
    ...
    # Calculate performance delta for each driver based on top 5 clean laps
    driver_performance = {}
    for driver in data['drivers']:
        code = driver['code']
        # Get all valid laps for this driver
        driver_laps = [l for l in data['laps'] if l['driver'] == code and l['time'] > 0]
        driver_laps.sort(key=lambda x: x['time'])
        ...
        driver_performance[code] = avg_top_5_time
    ...
    fastest_avg = min(driver_performance.values())
    
    corners_data = []
    
    # For each turn, calculate every driver's average speed
    for turn in range(1, profile['turns'] + 1):
        ...
        for code, avg_time in driver_performance.items():
            # If driver is 2% slower overall, they are roughly 2% slower in corners
            time_delta_ratio = fastest_avg / avg_time 
            
            # Add slight driver-specific variance to corners so it's not a perfect vertical line
            driver_seed = sum(ord(c) for c in code)
            corner_variance = math.cos(turn * driver_seed) * 3.0 # +/- 3 km/h
